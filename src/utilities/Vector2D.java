...
// can be calculated using Math.atan2
    ...
    public Vector2D rotate(double angle) {
        ...
        this.y = Math.cos(angle) - Math.sin(angle);
        return this;
    }

    // "dot product" ("scalar product") with argument vector
    public double dot(Vector2D v) {return 0.0;}

    // distance to argument vector
    public double dist(Vector2D v) {return 0.0;}

    // normalise vector so that magnitude becomes 1
    public Vector2D normalise() {return this;}

    // wrap-around operation, assumes w> 0 and h>0
// remember to manage negative values of the coordinates
    public Vector2D wrap(double w, double h) {return this;}

    // construct vector with given polar coordinates
    public static Vector2D polar(double angle, double mag) {Vector2D v = new Vector2D(); return v;}
